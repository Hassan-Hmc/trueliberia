...
<Icon name="bars" size={26}  color="#202020" />
    ...
    value={search}
        ...
        onChangeText={(searchString) => {setSearch(searchString); setSearchTrue(true); }}
...
myitems.map((v,i)=>{
   if(i<7){
    return(
      <View style={{width:'90%',paddingLeft:'10%',height:40,backgroundColor:'#fff',display:'flex',alignItems:'flex-start',justifyContent:'center',marginTop:0}} key={i} onTouchEnd={()=>{setSearch(v.name); setSearchTrue(false); props.navigation.navigate('searchDetails',{val:v.name})}}>
